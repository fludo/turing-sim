 ...
 * @author Ludovic Favre <ludovic.favre@epfl.ch>
 */
public class Inc implements TInstruction {
    ...
    public Inc(String[] input, ITypes[] types) throws InstructionParameterException,Exception {
    ...
    public boolean accepts() {
        //System.out.println("Accepts called with tape value : "+this.tm_.tapeString());
        return this.tm_.accepting();
    }

    @Override
    public boolean rejects() {
        return this.tm_.rejecting() || (!this.tm_.accepting());
    }
    ...
    public void paramsAre(String[] input, ITypes[] types) throws InstructionParameterException {
        if (input.length != this.inSize_) {
            throw new InvalidParameterException("Adder can only increment one argument at a time");
        }
    ...
    public void execute() throws Exception {
        ...
        SimpleTapeRunner r = new SimpleTapeRunner(tm_);
        while(r.doStep()){};
